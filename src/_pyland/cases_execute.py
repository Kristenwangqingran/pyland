...
class ExecTests(object):
    ...
    def run_test(self, case_nodes):
        """2 运行测试用例"""
        self.logger.info("--- start running cases---")
        self.logger.debug("the case running dir : %s" % (os.getcwd()))

        try:
            # 1. backup pytets.ini
            self.logger.info("---step 1. backup pytest.ini file ---")
            if os.path.exists("pytest.ini"):
                shutil.copy("pytest.ini", "pytest.ini.bak")
            # 2. update pytest.ini by case_nodes
            self.logger.info("---step 2. write case nodes to pytest.ini file ")
            self.logger.debug("the case nodes : %s" % ";".join([i for i in case_nodes.keys()]))
            self._pytest_config(case_nodes)
            self._debug_collection()
            # 3. execute testcases by pytets.ini
            self.logger.info("---step 3. batch running cases")
            self._run_pytest_cases()
        except Exception as e:
            self.logger.error(e)
        finally:
            # 4. recovery pytest.ini
            self.logger.info("---step 4. recover pytest.ini")
        ...
        self.logger.info("---step 5. get execution results")
    ...
    def _pytest_config(self, case_nodes):
        ...
        with open("pytest.ini") as f:
            rs = f.read()
        self.logger.debug("the pytest.ini file content is that" + "#" * 20 + "\n" + rs + "\n" + " + " * 20)
    ...
    def _run_pytest_cases(self):
        ...
        # else:
        PYTEST_PARAM.extend(['--tb=short', '-s', '-n', 'auto'])
        ...
        with Popen(RUNCMD,
            ...
            # fix deadlock due to subprocess.Popen -- ruanbin
            stdout, stderr = p.communicate()
            print(stdout, stderr)
            # for line in p.stderr:
            #     print(line)
        self.logger.info('generate allure result and report')
    ...
    def _get_case_results(self, case_nodes):
        ...
        self._clear_duplicated_result(REPORT_CASE_PATH)
        case_result = {}
        case_result_simple = {}
    ...
    def _clear_duplicated_result(self, report_case_path):
        all_tests = []
        self.logger.info(f'report case path: {report_case_path} \n\n')
        for test_path in os.listdir(report_case_path):
            # adding path + content
            res = Config(os.path.join(report_case_path, test_path)).json_get()
            all_tests.append([test_path, res])
        ...
        #         json_x1 = x[1]
        #         json_y1 = y[1]
        #         if json_x1['name'] == json_y1['name'] and json_x1['historyId'] == json_y1['historyId']:
        #             if json_x1['status'] == 'passed' and (json_y1['status'] == 'failed' or json_y1['status'] == 'broken'):
        #                 to_del.append(y)

        # another way to delete retried result and remain the last run result
        for x in all_tests:
            json_x1 = x[1]
            if json_x1['retry']:
                to_del.append(x)
        # self.logger.info(f'To delete result list: {to_del} \n\n')
        for t in to_del:
            r_path = os.path.join(report_case_path, t[0])
            if os.path.exists(r_path):
                os.remove(r_path)
                self.logger.info(f'Deleting file {r_path}')
            else:
                self.logger.info('File already deleted')
    ...
    def _debug_collection(self):
        rs = os.popen("python3 -m pytest --collect-only -q")
        raw_info = rs.read()
        results = [i for i in raw_info.split("\n") if '::' in i]
        self.logger.debug("total counted test %d" % len(results))
        for i, v in enumerate(results):
            self.logger.debug("[case  %d] is %s " % (i, v))
