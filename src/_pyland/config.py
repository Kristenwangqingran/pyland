...
class YamlParam(Config):
    ...
    @staticmethod
    def scan_all(src_dict: dict) -> list:
        if not isinstance(src_dict, dict):
            return src_dict
        value_lists = [src_dict]
        for k, v in src_dict.items():
            if isinstance(v, (list, tuple)):
                len_cur = len(value_lists)
                for _ in range(len_cur):
                    tmp = value_lists.pop(0)
                    for i in v:
                        cur = {}
                        cur.update(tmp)
                        if v is not None:
                            ...
                            cur.pop(k)
                        value_lists.append(cur)
            elif isinstance(v, dict):
                cur_v = YamlParam.scan_all(v)
                len_cur = len(value_lists)
                for _ in range(len_cur):
                    tmp = value_lists.pop(0)
                    for i in cur_v:
    ...
    def combined_params(self, all_combin=False):
        """
        ...
        try:
            ...
            for key in all_params.keys():
                if key.startswith("param"):
                    ...
                    if all_combin:
                        try:
                            all_combinations = self.scan_all(self.get(key))
                        except:
                            all_combinations = {}
                        res.update({key: all_combinations})
...
def com_params(yaml_files_list=None, path=None, strict=True,all_combin=False):
    """
    ...
    for file in yaml_files_list:
        yp = YamlParam(file)
        combined = yp.combined_params(all_combin=all_combin)
        if strict:
